		  	       ...
		  	       if rising_edge(clk) then 
		  	               if cnt < NBR_PIXELS then
		  	                       ...
		  	                       if cnt = 0 then -- ABS & add operations
                                      ...
                                      if (pixel >= weight(number_neurons, cnt)) then result := resize(result + resize((pixel - weight(number_neurons, cnt)), result), result);
                                      else result := resize(result + resize(-(pixel - weight(number_neurons, cnt)), result), result);
                                      end if;

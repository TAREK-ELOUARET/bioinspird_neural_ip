...
--    function linearize_2D_array(Azimuth_neuron :     array_2D -- linearize the matrix neurons into one dimension in order to facilitate its sort! // COmment it for SECOND VERSION
...
--    function maximum_set_SWM(Azimuth_neuron :     table_2D_to_1D; SWM_neurons_flag_learning: std_logic_type -- calculation of maximum neuron value to get updated its weight connection! --- FIRST VERSION OF MAXIMUM CALCULATION 
--                      ) return  sfixed is 
--                      variable temp : sfixed(SIZE_WIDTH_T -1 downto -6);
--                      variable Azimuth_neuron_t : table_2D_to_1D; -- table_azimuth 
--                      variable k : natural := 0;
            ...
            if flag_learning = '1' then  -- There will be a learning phase, and we deal just with the first maximum lines of WTA (signature) !
		          for i in 0 to NUMBER_OF_ACTIVE_NEURONS_SIGNATURE - 1 loop 
                        ...
                        for j in 0 to NUMBER_OF_column_NEURONS -1 loop
		                    ...
		                    SWM_neurons_t(to_integer(unsigned(number_of_line_max_WTA(i))),j) := activate_function((signature_neurons(to_integer(unsigned(number_of_line_max_WTA(i)))) * signature_weight_t(to_integer(unsigned(number_of_line_max_WTA(i))),j)) * (maximum_set_azimuth(Azimuth_neuron_t, l)));	                    
		                    l := l+3;
		                end loop;
		          end loop;
		          flag_SWM := '1';
                    ...
                    --for i in 0 to NUMBER_OF_ACTIVE_NEURONS_SIGNATURE - 1 loop 
                            ...
                            for j in 0 to NUMBER_OF_column_NEURONS -1 loop
                                          ...
                                          SWM_neurons_t(to_integer(unsigned(number_of_line_max_WTA(0))),j) := activate_function(SWM_neurons_t(to_integer(unsigned(number_of_line_max_WTA(0))),j) + (signature_neurons(to_integer(unsigned(number_of_line_max_WTA(0)))) * signature_weight_t(to_integer(unsigned(number_of_line_max_WTA(0))),j)) * (maximum_set_azimuth(Azimuth_neuron_t, l)));                        
                                          l := l+3;
                            end loop;
                    --end loop;
                    flag_SWM := '1';
...
--	learning:process (clk, flag_learning, flag_SWM) --flag_SWM,
--    variable Maximum_learning_value: sfixed(SIZE_WIDTH_T -1 downto -6);
--   -- variable find_coordonate_t: find_coordonate_type;
--    variable i,j : natural;
--    begin 
...
--                     -- UPDATE NOW THE WEIGHT OF THE SELECTED NEURON WITH MAXIMUM VALUE!  
--                     -- UPDATE the signal weight flag of neurons to not compete in the next itteration while reset = 0 for same image!!  
--                     signature_weight_t(find_coordonate(Maximum_learning_value, SWM_neurons_t, SWM_neurons_flag_learning)(0), find_coordonate(Maximum_learning_value, SWM_neurons_t, SWM_neurons_flag_learning)(1)) <= "0000000000000001000000";--Through coordonate i and J, we update the weight of the selected competeor neuron!!
